...
static double sqx(double x)
{
    return sin(x)/x;
}

static double sq_rfa(double q, double N_a, double D_a)
{
    const double tempx=q*D_a;
    return (1-sqx(tempx))/(1+sqx(tempx)) - 2*sqx(tempx)*(1-pow(sqx(tempx), N))/(N*pow(sqx(tempx), 2));
}
   ...
   double wamw)
{
    ...
    const double Sq = sq_rfa(q, N_a, D_a);
    return 1e-4 * volume * pow(contrast, 2) * pwlc * pcs / (1 + parav * cq * pwlc);
}